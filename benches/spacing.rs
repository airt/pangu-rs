...
extern crate test; // keep it for ide
...
#[bench]
fn simple_spaced(b: &mut test::Bencher) {
    b.iter(|| pangu::spacing("abc あ 123"));
}

#[bench]
fn sentence(b: &mut test::Bencher) {
    b.iter(|| pangu::spacing("新八的構造成分有95%是眼鏡、3%是水、2%是垃圾"));
}

#[bench]
fn sentence_spaced(b: &mut test::Bencher) {
    b.iter(|| pangu::spacing("新八的構造成分有 95% 是眼鏡、3% 是水、2% 是垃圾"));
}

#[bench]
fn paragraph(b: &mut test::Bencher) {
...
#[bench]
fn paragraph_spaced(b: &mut test::Bencher) {
