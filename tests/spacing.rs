...
fn text() {
    ...
    assert_eq!(
        pangu::spacing("所以,請問Jackey的鼻子有幾個?3.14個!"),
        "所以, 請問 Jackey 的鼻子有幾個? 3.14 個!"
    );
    assert_eq!(
        pangu::spacing("JUST WE就是JUST WE，既不偉大也不卑微！"),
        "JUST WE 就是 JUST WE，既不偉大也不卑微！"
    );
    assert_eq!(
        pangu::spacing("搭載MP3播放器，連續播放時數最長達到124小時的超強利刃……菊一文字RX-7!"),
        "搭載 MP3 播放器，連續播放時數最長達到 124 小時的超強利刃…… 菊一文字 RX-7!"
    );
...
fn hash() {
    ...
    assert_eq!(
        pangu::spacing("前面#銀河便車指南 後面"),
        "前面 #銀河便車指南 後面"
    );
    assert_eq!(
        pangu::spacing("前面#銀河便車指南tail"),
        "前面 #銀河便車指南 tail"
    );
    assert_eq!(
        pangu::spacing("前面#銀河公車指南 #銀河拖吊車指南 後面"),
        "前面 #銀河公車指南 #銀河拖吊車指南 後面"
    );
    assert_eq!(pangu::spacing("前面#H2G2#後面"), "前面 #H2G2# 後面");
    assert_eq!(
        pangu::spacing("前面#銀河閃電霹靂車指南#後面"),
        "前面 #銀河閃電霹靂車指南# 後面"
    );
}
...
fn parenthesis() {
    assert_eq!(
        pangu::spacing("前面(中文123漢字)後面"),
        "前面 (中文 123 漢字) 後面"
    );
    assert_eq!(pangu::spacing("前面(中文123)後面"), "前面 (中文 123) 後面");
    assert_eq!(pangu::spacing("前面(123漢字)後面"), "前面 (123 漢字) 後面");
    assert_eq!(
        pangu::spacing("前面(中文123漢字) tail"),
        "前面 (中文 123 漢字) tail"
    );
    assert_eq!(
        pangu::spacing("head (中文123漢字)後面"),
        "head (中文 123 漢字) 後面"
    );
    assert_eq!(
        pangu::spacing("head (中文123漢字) tail"),
        "head (中文 123 漢字) tail"
    );
}
...
fn quote() {
    ...
    assert_eq!(
        pangu::spacing("前面\"中文123\"後面"),
        "前面 \"中文 123\" 後面"
    );
    assert_eq!(
        pangu::spacing("前面\"123漢字\"後面"),
        "前面 \"123 漢字\" 後面"
    );
    assert_eq!(
        pangu::spacing("前面\"中文123漢字\" tail"),
...
fn single_quote() {
    ...
    assert_eq!(
        pangu::spacing("陳上進 likes 林依諾's status."),
